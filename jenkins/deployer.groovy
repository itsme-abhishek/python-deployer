deployer_image = "allvy/deployer:0.0.2"
...
def deployStandAlone(env, service, image, tag){
    stage("Deploy to ${env} env"){
        def cmd = "python scripts/apply.py  -e ${env} -m ${service} -i allvy/${image}:${tag} -dmi allvy/${image}-db:${tag} -conf -secret"
        run(env, cmd)
    }
}
...
def set_kube_credentials(env){
    ...
    if (env == "apUat" || env == "apProd" || env == "apSecfix") {
